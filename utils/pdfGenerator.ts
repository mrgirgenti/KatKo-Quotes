    ...
    <body>
      ...
      <div class="section">
        ...
        <table class="summary-table">
          ...
          ${(quote.calculations.discountAmount ?? 0) > 0 ? `
            <tr style="color:#dc2626;"><td class="label-col">Discount (${quote.calculations.discountType === 'percentage' ? `${(quote.calculations.discountValue ?? 0).toFixed(1)}%` : `$${(quote.calculations.discountValue ?? 0).toFixed(2)}`})${quote.discountData?.reason && quote.discountData.reason !== '' ? ` — ${quote.discountData.reason === 'custom' ? (quote.discountData.customReason || 'Custom') : quote.discountData.reason.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}` : ''}</td><td></td><td class="val-col" style="color:#dc2626;">-${formatCurrency(quote.calculations.discountAmount ?? 0)}</td></tr>
            <tr class="divider bold"><td class="label-col">Subtotal after Discount</td><td class="val-col">${formatCurrency(quote.calculations.totalQuantity > 0 ? (quote.calculations.discountedSubtotal ?? quote.calculations.subtotal) / quote.calculations.totalQuantity : 0)}/ea</td><td class="val-col">${formatCurrency(quote.calculations.discountedSubtotal ?? quote.calculations.subtotal)}</td></tr>
          ` : ''}
