...
interface LibraryManagementMenuProps {
  libraryName: string;
  /** Trigger color: 'dark' (white icon on black header) or 'light' (brand icon on light bg). */
  variant?: 'dark' | 'light';
  /** Called when user picks a file. Parse and validate; return preview. */
  onParseImport: (content: string, format: 'csv' | 'json') => LibraryImportPreview;
  /** Called when user confirms import with chosen mode. */
  onConfirmImport: (rows: any[], mode: ImportMode) => Promise<void>;
  /** Called to generate the export file content. */
  getExportData: (format: 'csv' | 'json') => string;
  /** Called to generate a blank import template (CSV). */
...
export default function LibraryManagementMenu({
  ...
  function pickFile(format: 'csv' | 'json') {
    if (typeof document === 'undefined') return;
    const input = document.createElement('input');
    input.type = 'file';
    input.accept = format === 'csv' ? '.csv,text/csv' : '.json,application/json';
    input.onchange = (e: Event) => {
      ...
      reader.onload = (ev) => {
        ...
        if (!content) return;
        const result = onParseImport(content, format);
        setPreview(result);
      ...
      >
        {({ close }) => (
          <>
            ...
            <TouchableOpacity
              style={mm.item}
              onPress={() => { close(); pickFile('json'); }}
              activeOpacity={0.7}
            >
              <Upload size={13} color={Colors.light.tint} />
              <Text style={mm.itemText}>Import JSON…</Text>
            </TouchableOpacity>
            ...
            >
              ...
              <Text style={mm.itemText}>Download Import Template</Text>
            </TouchableOpacity>
              ...
              >
                ...
                <View style={imp.summaryRow}>
                  <View style={imp.summaryItem}>
                    <Text style={[imp.summaryCount, { color: '#16A34A' }]}>
                      {preview.validCount}
                    </Text>
                  ...
                  <View style={imp.summaryItem}>
                    <Text
                      style={[
                        imp.summaryCount,
                        { color: preview.duplicateCount > 0 ? '#D97706' : Colors.light.textSecondary },
                      ]}
                    >
                  ...
                  <View style={imp.summaryItem}>
                    <Text
                      style={[
                        imp.summaryCount,
                        { color: preview.invalidCount > 0 ? Colors.light.error : Colors.light.textSecondary },
                      ]}
                    >
                      ...
                      [
                        {
                          ...
                          label: 'Append',
                          hint: 'Add all valid rows as new entries',
                        },
                        {
                          value: 'replace' as const,
                          label: 'Replace Existing',
                          hint: 'Update matching names; add the rest',
                        },
                        {
                          value: 'skip' as const,
                          label: 'Skip Duplicates',
                          hint: 'Ignore matching names; add the rest',
                        },
                      ]
                    ).map((opt) => (
                      <TouchableOpacity
                      ...
                      >
                        ...
                        <View style={{ flex: 1 }}>
                          <Text
                            style={[
                              imp.modeLabel,
                              importMode === opt.value && imp.modeLabelActive,
                            ]}
                          >
                            {opt.label}
                          </Text>
                    ...
                    {importMode === 'replace' && preview.duplicateCount > 0 && (
                      <View style={imp.warningBox}>
                        <Text style={imp.warningText}>
                          ⚠ This will overwrite {preview.duplicateCount} existing record
                          {preview.duplicateCount !== 1 ? 's' : ''}. This cannot be undone.
                        </Text>
                      </View>
                    )}
                  </>
                  ...
                  >
                    {importing ? (
                      ...
                      <Text style={imp.importBtnText}>
                        Import {preview.validCount} Record
