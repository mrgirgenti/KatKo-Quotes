...
export async function POST(request: Request) {
  ...
  const { styleNumber, vendor, brand, name, category, sortOrder, subcategory, productType, gender, recommendationLevel } = body as Record<string, string>;
  ...
  try {
    const result = await client.query(
      `INSERT INTO "Product" (id, "styleNumber", vendor, brand, name, category, subcategory, "productType", gender, "recommendationLevel", "isActive", "sortOrder", "createdAt", "updatedAt")
      ...
      [styleNumber.trim(), vendor.trim(), brand.trim(), name.trim(), category.trim(),
       ...
       recommendationLevel?.trim() || null, sortOrder ?? 0],
    );
